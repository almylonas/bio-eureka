...
def build_knowledge_graph():
    ...
    try:
        ...
        for i, row in df_sample.iterrows():
            ...
            # Add organism nodes
            for organism in entities["organisms"][:3]:
                org_key = f"organism_{organism.lower()}"
                if org_key not in node_id_map:
                    node_id_map[org_key] = node_counter
                    nodes.append({
                        "id": node_counter,
                        "label": organism,
                        "type": "organism",
            ...
            # Add condition nodes
            for condition in entities["conditions"][:3]:
                cond_key = f"condition_{condition.lower()}"
                if cond_key not in node_id_map:
                    node_id_map[cond_key] = node_counter
                    nodes.append({
                        "id": node_counter,
                        "label": condition,
                        "type": "condition",
            ...
            # Add outcome nodes
            for outcome in entities["outcomes"][:3]:
                out_key = f"outcome_{outcome.lower()}"
                if out_key not in node_id_map:
                    node_id_map[out_key] = node_counter
                    nodes.append({
                        "id": node_counter,
                        "label": outcome,
                        "type": "outcome",
        ...
        for edge in edges:
            ...
            if target_node and target_node["type"] == "concept":
                ...
                if concept_id not in concept_papers:
        ...
        return jsonify({
            ...
            "stats": {
                ...
                "organisms": len([n for n in nodes if n["type"] == "organism"]),
                "conditions": len([n for n in nodes if n["type"] == "condition"]),
                "outcomes": len([n for n in nodes if n["type"] == "outcome"])
            },
            "status": "success"
        })
    ...
    <body>
        <div class="tabs-container">
            <div class="tabs">
                ...
                <div class="tab" onclick="switchTab('kg')">🕸️ Knowledge Graph</div>
            </div>
        </div>
        ...
        <div class="container">
            ...
            <!-- Knowledge Graph Tab -->
            <div id="kgTab" class="tab-content">
                <h1>🕸️ Knowledge Graph Explorer</h1>
                
                <div class="note">
                    <strong>🧠 Interactive 3D Graph:</strong> Explore concepts, relationships, and papers in space biology research. 
                    Click nodes to view details, drag to rotate, scroll to zoom.
                </div>
                
                <div class="kg-stats">
                    <div class="kg-stat-card">
                        <div class="kg-stat-value" id="kgNodeCount">0</div>
                        <div class="kg-stat-label">NODES</div>
                    </div>
                    <div class="kg-stat-card">
                        <div class="kg-stat-value" id="kgEdgeCount">0</div>
                        <div class="kg-stat-label">CONNECTIONS</div>
                    </div>
                    <div class="kg-stat-card">
                        <div class="kg-stat-value" id="kgPaperCount">0</div>
                        <div class="kg-stat-label">PAPERS</div>
                    </div>
                    <div class="kg-stat-card">
                        <div class="kg-stat-value" id="kgConceptCount">0</div>
                        <div class="kg-stat-label">CONCEPTS</div>
                    </div>
                </div>
                
                <div style="display: flex; gap: 10px; margin-bottom: 20px; align-items: center;">
                    <label><strong>Papers to analyze:</strong></label>
                    <input type="number" id="kgPaperCount" value="30" min="5" max="100" style="width: 100px; padding: 8px; border: 2px solid var(--border-color); border-radius: 5px; background: var(--bg-tertiary); color: var(--text-primary);">
                    <button onclick="buildKnowledgeGraph()" id="buildKGBtn">🔨 BUILD GRAPH</button>
                    <button onclick="resetKnowledgeGraph()" style="background: #dc3545;">🔄 RESET</button>
                </div>
                
                <div id="kgLoading" class="loading" style="display: none;">
                    <div class="spinner"></div>
                    <div style="margin-top: 15px;">Building knowledge graph from papers...</div>
                </div>
                <div id="kgError" class="error" style="display: none;"></div>
                
                <div id="graphContainer">
                    <div class="graph-controls">
                        <button onclick="centerGraph()" title="Center View">🎯 Center</button>
                        <button onclick="togglePhysics()" id="physicsBtn" title="Toggle Physics">⏸️ Pause</button>
                        <button onclick="exportGraph()" title="Export Graph">💾 Export</button>
                    </div>
                    
                    <div class="graph-legend">
                        <strong style="display: block; margin-bottom: 10px;">Node Types</strong>
        ...
        <script>
            ...
            function switchTab(tabName) {
                ...
                if (tabName === 'fetch') {
                    ...
                    document.getElementById('kgTab').classList.add('active');
                    if (graph3D) {
                        setTimeout(() => graph3D.refresh(), 100);
                    }
                }
            }
            ...
            function initializeGraph(data) {
                ...
                if (graph3D) {
                    ...
                    container.innerHTML = `
                        ...
                        <div class="graph-legend">
                            <strong style="display: block; margin-bottom: 10px;">Node Types</strong>
                            <div class="legend-item"><div class="legend-color" style="background: #ff6b6b;"></div><span>Paper</span></div>
                            <div class="legend-item"><div class="legend-color" style="background: #4ecdc4;"></div><span>Concept</span></div>
                            <div class="legend-item"><div class="legend-color" style="background: #95e1d3;"></div><span>Organism</span></div>
                            <div class="legend-item"><div class="legend-color" style="background: #f38181;"></div><span>Condition</span></div>
            ...
            function showNodeInfo(node) {
                ...
                const contentEl = document.getElementById('nodeInfoContent');
            ...
            function togglePhysics() {
                ...
                btn.textContent = physicsRunning ? '⏸️ Pause' : '▶️ Play';
            ...
            function resetKnowledgeGraph() {
                if (confirm('Are you sure you want to reset the knowledge graph?')) {
                    const container = document.getElementById('graphContainer');
                    container.innerHTML = `
                        <div class="graph-controls">
                            <button onclick="centerGraph()" title="Center View">🎯 Center</button>
